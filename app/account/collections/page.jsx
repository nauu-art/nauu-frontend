...
import { Plus, Trash2, Lock, Globe, Package, Star } from 'lucide-react'
...
export default function CollectionsPage() {
  ...
  const handleCreate = async (e) => {
    ...
    setCreating(true)
    try {
      await api.post('/collections', { name: newName, description: newDesc, isPublic: true })
  ...
  const handleTogglePublic = async (id, isPublic) => {
    try {
      await api.put(`/collections/${id}`, { isPublic: !isPublic })
      setCollections(prev => prev.map(c => c.id === id ? {...c, isPublic: !isPublic} : c))
      toast.success(!isPublic ? 'Baú público' : 'Baú privado')
    ...
    <div className="p-5 md:p-8">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-extrabold text-gray-900">Os meus baús</h1>
              <p className="text-sm text-gray-400 font-medium mt-0.5">{collections.length} baús · clica ou mantém pressionado ⚓ para adicionar obrass</p>
            </div>
          </div>
        ...
        {showForm && (
          <form onSubmit={handleCreate} className="bg-white border border-gray-100 rounded-2xl p-5 mb-6">
            <h2 className="text-sm font-extrabold text-gray-900 mb-4">Novo baú</h2>
            <div className="flex flex-col gap-3">
              <div>
                <label className="label">Nome *</label>
                <input value={newName} onChange={e => setNewName(e.target.value)} className="input" placeholder="Ex: Arte Abstrata, Viagens 2026…" required />
              </div>
              <div>
        ...
        {loadingColls ? (
          ...
          <div className="text-center py-20 bg-white rounded-2xl border border-gray-100">
            ...
            <div className="text-gray-300 font-bold text-lg mb-2">Ainda não tens baús</div>
            <p className="text-gray-400 text-sm font-medium mb-6">Cria baús para organizar as obras que mais gostas.</p>
            <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-500 hover:bg-blue-600 text-white font-bold text-sm rounded-xl transition-colors">
              <Plus size={16} /> Criar primeiro baú
            </button>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between mb-1">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={e => { e.preventDefault(); e.stopPropagation(); handleTogglePublic(col.id, col.isPublic) }}
                        title={col.isPublic ? 'Baú público — clica para tornar privado' : 'Baú privado — clica para tornar público'}
